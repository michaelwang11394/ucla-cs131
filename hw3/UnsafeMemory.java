...
//   starts to be problematic: program cannot finish

// java UnsafeMemory Synchronized 8 100000000 10 5 6 3 0 3
// java UnsafeMemory GetNSet      8 100000000 10 5 6 3 0 3
// GetNSet seems to be reliable, and slightly faster

// Infinite loop here is a possibility, as every element in the array could drop below 0 
// (think of 1 element starting as 1, two threads passed the condition, and do the minus, and the element will keep being smaller than 0)
...
class CheckTask extends TimerTask {
    ...
    CheckTask(byte[] v, byte m) { value = v; maxval = m; }

    public void run() {
        /*
        for (int i = 0; i < value.length; i++) {
            if (value[i] > maxval || value[i] < 0) {
                System.err.println("Execution stopped by timer");
                System.exit (1);
            }
        }
        */
        System.err.println("Execution times out.");
        System.exit (1);
    }
}
...
class UnsafeMemory {
    public static void main(String args[]) {
        ...
        try {
            ...
            timer.schedule(new CheckTask(stateArg, maxval), 30000);
